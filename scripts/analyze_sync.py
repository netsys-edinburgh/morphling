...
def organize_by_gemm(events: List[ComputeEvent]) -> Dict[int, List[ComputeEvent]]:
...
def calculate_sync_metrics(
    ...
    for gemm_id in original_events.keys():
        ...
        # Calculate spread
        orig_spread = max(orig_vt_starts.values()) - min(orig_vt_starts.values())
        sync_spread = max(sync_vt_starts.values()) - min(sync_vt_starts.values())
        improvement = orig_spread - sync_spread
        improvement_pct = (improvement / orig_spread * 100) if orig_spread > 0 else 0
...
def print_comparison(metrics: Dict[int, Dict]):
    ...
    print(f"  Successfully synchronized: {synced_count} ({synced_count*100//total_gemm}%)")
    ...
    for gemm_id in sorted(metrics.keys()):
        ...
        # Improvement
        status = "✓ SYNCHRONIZED" if m['is_synchronized'] else "✗ NOT SYNCHRONIZED"
        print(f"  Status: {status}")
        print(f"  Improvement: {m['improvement_us']:>6} us ({m['improvement_pct']:>6.1f}%)")
...
def print_timeline(metrics: Dict[int, Dict]):
    ...
    for gemm_id in sorted(metrics.keys())[:5]:  # Show first 5 GEMM ops
        ...
        for dev_id in m['devices']:
            ...
            bar = '─' * bar_length + '●'
            print(f"  Device {dev_id}: {bar} {vt} us (offset: +{offset_from_min} us)")
        ...
        for dev_id in m['devices']:
            ...
            print(f"  Device {dev_id}: {bar} {vt} us (offset: +{offset_from_min} us) {status}")
