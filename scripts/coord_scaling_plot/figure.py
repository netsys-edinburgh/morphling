...
def _panel_device(ax, device: schema.DeviceScaling) -> None:
    ...
    line_rt = ax.plot(
        counts,
        runtimes,
        color=_RUNTIME_COLOR,
        marker="o",
        markersize=3.5,
        label="Iteration runtime",
    )
    twin = ax.twinx()
    line_cpu = twin.plot(
        counts,
...
def _plot_efficiency(
    ax,
    efficiency: schema.ScalingEfficiency,
    color: str,
    marker: str,
    style: str,
) -> None:
    ...
    ax.plot(
        ...
        ls=style,
        markersize=4,
        label=efficiency.mode.capitalize(),
    )


def _panel_efficiency(
    ax, strong: schema.ScalingEfficiency, weak: schema.ScalingEfficiency
) -> None:
    ...
    ticks = sorted(
        {point.coordinators for point in (*strong.points, *weak.points)}
    )
    ax.set_xticks(ticks)
...
def _panel_breakdown(ax, breakdown: schema.Breakdown) -> None:
    ...
    labels = [
        ("S" if row.mode == "strong" else "W") + str(row.coordinators)
        for row in rows
    ]
    ...
    for index, component_label in enumerate(COMPONENT_LABELS):
        ...
        ax.bar(
            ...
            bottom=bottoms,
            width=0.7,
            color=_COMPONENT_COLORS[index],
            hatch=_COMPONENT_HATCHES[index],
            edgecolor="black",
            linewidth=0.4,
...
def _assemble(
    device: schema.DeviceScaling,
    strong: schema.ScalingEfficiency,
...
def build_figure(
    ...
    with paper_style(
        width=_FIG_WIDTH_INCHES, height=_FIG_HEIGHT_INCHES, font_size=7
    ):
