...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--strong", required=True, type=Path,
                        help="strong-scaling strong.json")
    parser.add_argument("--weak", required=True, type=Path,
                        help="weak-scaling weak.json")
    parser.add_argument("--breakdown", required=True, type=Path,
...
def _load_inputs(args: argparse.Namespace):
    ...
    strong = schema.parse_scaling_efficiency(schema.load_json(args.strong), "strong")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    pdf_path, png_path = figure.render(device, strong, weak, breakdown, args.out)
