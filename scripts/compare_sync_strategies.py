...
def parse_log_for_compute_times(log_file: str) -> Dict[int, Dict[int, Tuple[int, int]]]:
    """
        ...
        for line in f:
            ...
            try:
                ...
                if event == 'START' and gemm_id not in compute_times[device_id]:
                    ...
                    # 记录 END 时间的 vt_end
                    if gemm_id in compute_times and device_id in compute_times[gemm_id]:
...
def analyze_strategy_differences(compute_times: Dict[int, Dict[int, Tuple[int, int]]]) -> Tuple[Dict, Dict]:
    """
    分析两种策略的差异
    ...
    for gemm_id in sorted(compute_times.keys()):
        ...
        offsets_earliest = {d: baseline_earliest - vt_starts[d] for d in vt_starts.keys()}
        
        # ============ LATEST 策略 ============
        baseline_latest = max(vt_starts.values())
        offsets_latest = {d: baseline_latest - vt_starts[d] for d in vt_starts.keys()}
        ...
        if vt_ends:
            ...
            fastest_device = min(compute_durations.items(), key=lambda x: x[1])[0] if compute_durations else None
        else:
            fastest_device = None
...
def print_detailed_comparison(earliest: Dict, latest: Dict, compute_times: Dict):
    """打印详细对比分析"""
...
""")
    ...
    for gemm_id in range(0, min(5, len(earliest))):
        ...
        print(f"│   Baseline: {early['baseline']} us (Device {early['baseline_device']} 最快)")
        print(f"│   同步后的虚拟时间:")
        for device_id in sorted(vt_starts.keys()):
            ...
            print(f"│     Device {device_id}: {new_vt:>12} us  (offset={offset:>7} {status})")
        
        # LATEST 策略
        print(f"│")
        print(f"│ 【LATEST 策略 - 以最慢的设备为基准】")
        print(f"│   Baseline: {late['baseline']} us (Device {late['baseline_device']} 最慢)")
        print(f"│   同步后的虚拟时间:")
        for device_id in sorted(vt_starts.keys()):
            offset = late['offsets'][device_id]
            new_vt = vt_starts[device_id] + offset
            status = "基准 (0)" if offset == 0 else f"加快 ({offset:>+7})" if offset > 0 else f"拖慢 ({offset})"
            print(f"│     Device {device_id}: {new_vt:>12} us  (offset={offset:>+7} {status})")
        ...
        print(f"│     • 影响范围: {'LATEST 更大' if late['max_offset'] > early['max_offset'] else 'EARLIEST 更大'}")
...
def print_statistical_comparison(earliest: Dict, latest: Dict):
    ...
    earliest_baseline_devices = [v['baseline_device'] for v in earliest.values()]
    latest_baseline_devices = [v['baseline_device'] for v in latest.values()]
    
    print(f"\n【Baseline 虚拟时间】")
    print(f"├─ EARLIEST strategy:")
    print(f"│  ├─ 平均值: {sum(earliest_baselines)/len(earliest_baselines):>12.0f} us")
    ...
    print(f"   ├─ 平均值: {sum(latest_baselines)/len(latest_baselines):>12.0f} us")
    print(f"   ├─ 最小值: {min(latest_baselines):>12} us")
    print(f"   ├─ 最大值: {max(latest_baselines):>12} us")
    print(f"   └─ 特点: 总是选择最大的虚拟时间（最慢的设备）")
    
    baseline_shift = sum(latest_baselines) / len(latest_baselines) - sum(earliest_baselines) / len(earliest_baselines)
    print(f"\n【虚拟时间位移】")
    print(f"├─ 平均位移: {baseline_shift:>12.0f} us (LATEST 比 EARLIEST 向前推)")
    print(f"└─ 此位移对所有设备影响相同，不影响相对关系")
    
    print(f"\n【最大偏移量（设备需要调整的幅度）】")
    print(f"├─ EARLIEST:")
    print(f"│  ├─ 平均值: {sum(earliest_max_offsets)/len(earliest_max_offsets):>12.0f} us")
    print(f"│  ├─ 最小值: {min(earliest_max_offsets):>12} us")
    print(f"│  └─ 最大值: {max(earliest_max_offsets):>12} us")
    print(f"│")
    print(f"└─ LATEST:")
    print(f"   ├─ 平均值: {sum(latest_max_offsets)/len(latest_max_offsets):>12.0f} us")
