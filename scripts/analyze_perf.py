...
def calculate_peak_throughput(stats, direction):
    ...
    for device_id, device_stats in stats.items():
        ...
        for i, tp in enumerate(tp_list):
            if i < len(epoch_starts) and i < len(epoch_ends):
                ...
                packet_size = 0
                if direction == 'upload' and i < len(device_stats['upload_bytes']):
                    packet_size = device_stats['upload_bytes'][i]
                elif direction == 'download' and i < len(device_stats['download_bytes']):
                ...
                if packet_size > 0:
                    ...
                    events.append((start_us, 'start', device_id, tp_b_s, duration_s))
                    events.append((end_us, 'end', device_id, tp_b_s, duration_s))
    ...
    events.sort(key=lambda x: (x[0], x[1] == 'start'))  # 'end' (False) comes before 'start' (True)
...
def analyze_perf_log(log_file, skip_warmup_seconds=0, target_devices=None, direction_filter=None):
    ...
    try:
        with open(log_file, 'r') as f:
            ...
            if skip_warmup_seconds > 0:
                ...
                filtered_stats = defaultdict(lambda: {
                    'upload': [],
                    'download': [],
                    'upload_bytes': [],
                    'download_bytes': [],
                    ...
                    for i, ts in enumerate(device_stats['timestamps']):
                        if ts >= global_start_us + warmup_us:
                            if device_stats['upload']:
                                filtered_stats[device_id]['upload'].append(device_stats['upload'][i])
                                filtered_stats[device_id]['upload_bytes'].append(device_stats['upload_bytes'][i])
                            if device_stats['download']:
                                filtered_stats[device_id]['download'].append(device_stats['download'][i])
                                filtered_stats[device_id]['download_bytes'].append(device_stats['download_bytes'][i])
                            filtered_stats[device_id]['timestamps'].append(ts)
                            filtered_stats[device_id]['packet_durations'].append(device_stats['packet_durations'][i])
                            filtered_stats[device_id]['epoch_starts'].append(device_stats['epoch_starts'][i])
                            filtered_stats[device_id]['epoch_ends'].append(device_stats['epoch_ends'][i])
            ...
            for device_id in sorted(stats.keys(), key=lambda x: int(x)):
                ...
                if device_stats['timestamps']:
                    ...
                    device_duration_s = (device_end - device_start) / 1_000_000.0
                    print(f"  Runtime: {device_duration_s:.2f} seconds")
                else:
                    device_duration_s = 0
                
                device_info = {'upload_bytes': 0, 'download_bytes': 0, 'runtime_s': device_duration_s}
                ...
                if device_stats['upload']:
                    ...
                    if device_duration_s > 0:
                        ...
                        print(f"  Upload:")
                        print(f"    Total: {upload_bytes_total / 1024 / 1024:.2f} MB")
                        print(f"    Packets: {len(upload_tp)}")
                        print(f"    Rate: {upload_rate / 1024 / 1024:.2f} MB/s")
                        print(f"    Per-packet TP - Min: {min(upload_tp) / 1024 / 1024:.2f} MB/s, Max: {max(upload_tp) / 1024 / 1024:.2f} MB/s")
                        print(f"    Per-packet TP - Mean: {mean(upload_tp) / 1024 / 1024:.2f} MB/s, Median: {median(upload_tp) / 1024 / 1024:.2f} MB/s")
                        if len(upload_tp) > 1:
                            print(f"    Per-packet TP - StdDev: {stdev(upload_tp) / 1024 / 1024:.2f} MB/s")
                ...
                if device_stats['download']:
                    ...
                    if device_duration_s > 0:
                        ...
                        print(f"  Download:")
                        print(f"    Total: {download_bytes_total / 1024 / 1024:.2f} MB")
                        print(f"    Packets: {len(download_tp)}")
                        print(f"    Rate: {download_rate / 1024 / 1024:.2f} MB/s")
                        print(f"    Per-packet TP - Min: {min(download_tp) / 1024 / 1024:.2f} MB/s, Max: {max(download_tp) / 1024 / 1024:.2f} MB/s")
                        print(f"    Per-packet TP - Mean: {mean(download_tp) / 1024 / 1024:.2f} MB/s, Median: {median(download_tp) / 1024 / 1024:.2f} MB/s")
                        if len(download_tp) > 1:
                            print(f"    Per-packet TP - StdDev: {stdev(download_tp) / 1024 / 1024:.2f} MB/s")
                ...
                if device_stats['packet_durations']:
                    ...
                    print(f"    Mean: {mean(durations):.2f}, Median: {median(durations):.2f}")
                    if len(durations) > 1:
            ...
            total_upload_bytes = sum(info['upload_bytes'] for info in device_summary.values())
            total_download_bytes = sum(info['download_bytes'] for info in device_summary.values())
            ...
            if total_upload_bytes > 0:
                ...
                print(f"Upload:")
                print(f"  Total Bytes: {total_upload_bytes / 1024 / 1024:.2f} MB")
                print(f"  Average TP: {server_upload_tp / 1024 / 1024:.2f} MB/s")
            
            if total_download_bytes > 0:
                server_download_tp = total_download_bytes / global_duration_s
                print(f"Download:")
                print(f"  Total Bytes: {total_download_bytes / 1024 / 1024:.2f} MB")
                print(f"  Average TP: {server_download_tp / 1024 / 1024:.2f} MB/s")
            ...
            # Calculate peak concurrent throughput (considering packet timing overlaps)
            print(f"\n{'='*80}")
            print("Peak Concurrent Throughput (considering packet timing overlaps):")
            print(f"{'='*80}\n")
            
            peak_upload_tp, peak_upload_time = calculate_peak_throughput(stats, 'upload')
            peak_download_tp, peak_download_time = calculate_peak_throughput(stats, 'download')
            peak_total_tp = peak_upload_tp + peak_download_tp
            
            print(f"Peak Upload TP: {peak_upload_tp / 1024 / 1024:.2f} MB/s at {peak_upload_time:.6f}s")
            print(f"Peak Download TP: {peak_download_tp / 1024 / 1024:.2f} MB/s at {peak_download_time:.6f}s")
            print(f"Peak Total TP: {peak_total_tp / 1024 / 1024:.2f} MB/s")
            
            if total_bytes > 0:
                peak_ratio = peak_total_tp / server_total_tp if server_total_tp > 0 else 0
            ...
            print(f"{'='*80}\n")
            print(f"{'Device':<8} {'Upload(MB)':<12} {'Download(MB)':<14} {'Runtime(s)':<12} {'UL Rate(MB/s)':<15} {'DL Rate(MB/s)':<15}")
            print("-" * 100)
            
            for device_id in sorted(device_summary.keys(), key=lambda x: int(x)):
                ...
                print(f"{device_id:<8} {ul_mb:<12.2f} {dl_mb:<14.2f} {runtime:<12.2f} {ul_rate:<15.2f} {dl_rate:<15.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('log_file', nargs='?', default='./perf.log', help='Path to perf.log file')
    parser.add_argument('--skip-warmup', type=float, default=0, help='Skip first N seconds (warmup period)')
    parser.add_argument('--devices', type=str, help='Analyze only specific devices (comma-separated, e.g., "0,1,2")')
    parser.add_argument('--direction', choices=['upload', 'download'], help='Filter by direction')
    ...
    result = analyze_perf_log(args.log_file, skip_warmup_seconds=args.skip_warmup, 
                             target_devices=target_devices, direction_filter=args.direction)
