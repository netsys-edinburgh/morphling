...
def plot_timeline(timeline, devices, use_vtime=True, title="Timeline", filename=None, 
    ...
    for device_id in devices:
        ...
        for event in events:
            ...
            # Add event label
            if clipped_duration > 3000:  # Only label if visible duration > 3000 us
    ...
    ax.set_yticklabels([f'Device {d}' for d in devices], fontsize=13, fontweight='bold')
    ...
    ax.set_xticklabels(tick_labels, rotation=45, ha='right', fontsize=11, fontweight='bold')
    ...
    ax.grid(True, axis='x', which='major', alpha=0.6, linestyle='-', linewidth=1.0, color='gray')
    ax.grid(True, axis='x', which='minor', alpha=0.2, linestyle=':', linewidth=0.5, color='lightgray')
    ax.grid(True, axis='y', alpha=0.3, linestyle='--', linewidth=0.8, color='gray')
    ax.set_axisbelow(True)
    
    # Create legend
    legend_elements = [
        mpatches.Patch(facecolor=EVENT_COLORS['COMPUTE'], label='COMPUTE', edgecolor='black', linewidth=1),
        mpatches.Patch(facecolor=EVENT_COLORS['DOWNLOAD'], label='DOWNLOAD', edgecolor='black', linewidth=1),
        mpatches.Patch(facecolor=EVENT_COLORS['UPLOAD'], label='UPLOAD', edgecolor='black', linewidth=1),
        mpatches.Patch(facecolor=EVENT_COLORS['SEND'], label='SEND', edgecolor='black', linewidth=1),
        mpatches.Patch(facecolor=EVENT_COLORS['RECEIVE'], label='RECEIVE', edgecolor='black', linewidth=1),
    ]
    ax.legend(handles=legend_elements, loc='upper right', fontsize=12, framealpha=0.98, edgecolor='black', 
             title='Event Types', title_fontsize=12, frameon=True, fancybox=True)
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python3 timeline_plot.py perf_merged_synced.log . --gemm-range 0:5")
        print("  python3 timeline_plot.py perf_merged_synced.log . --gemm-range 5:10")
        print("\nThis will generate:")
