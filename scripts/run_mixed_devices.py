...
def parse_args():
    parser = argparse.ArgumentParser(description="Run mixed real and fake devices")
    
    # Device configuration
    parser.add_argument("--num_devices", type=int, default=3,
                        help="Total number of devices to run")
    parser.add_argument("--num_fake_devices", type=int, default=2,
    ...
    parser.add_argument("--device_flops", type=float, default=1e12,
                        help="FLOPS per device")
    parser.add_argument("--device_mem", type=int, default=8,
                        help="Memory per device (GB)")
    parser.add_argument("--ul_bw", type=float, default=100,
                        help="Upload bandwidth (Mbps)")
    parser.add_argument("--dl_bw", type=float, default=100,
                        help="Download bandwidth (Mbps)")
    parser.add_argument("--ul_lat", type=float, default=10,
                        help="Upload latency (ms)")
    parser.add_argument("--dl_lat", type=float, default=10,
                        help="Download latency (ms)")
...
class FakeDevice:
    ...
    def __init__(self, device_id: int, host: str = "127.0.0.1", port: int = 50051):
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"[Fake Device {self.device_id}] Connected to {self.host}:{self.port}")
            return True
        ...
        while self.running:
            try:
                ...
                while len(task_data) < task_size:
                    chunk = self.socket.recv(min(task_size - len(task_data), 4096))
                    if not chunk:
                        break
                    task_data += chunk
...
def print_real_device_command(device_id: int, args):
    ...
    command = [
        ...
        "--ul_bw", ul_bw_str,
        "--dl_bw", dl_bw_str,
        "--ul_lat", str(args.ul_lat),
        "--dl_lat", str(args.dl_lat),
...
def main():
    ...
    if args.num_fake_devices > args.num_devices:
        print(f"Error: num_fake_devices ({args.num_fake_devices}) cannot be greater than num_devices ({args.num_devices})")
        sys.exit(1)
    ...
    if args.backend == "proxy":
        ...
        print(f"  - Waiting for {num_real_devices} real device(s) to connect manually\n")
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if connection_count != last_count:
                    fake_connected = min(connection_count, args.num_fake_devices)
                    real_connected = max(0, connection_count - args.num_fake_devices)
                    print(f"Connected: {connection_count}/{args.num_devices} "
            ...
            connected = backend.get_connection_count() if hasattr(backend, 'get_connection_count') else 0
    ...
    # Prepare input
    print("\n=== Running Inference ===")
    input_text = ["Hello, my dog is cute. He is a good " * 128] * args.batch_size
    inputs = tokenizer(
        input_text,
        return_tensors="pt",
        padding=True,
