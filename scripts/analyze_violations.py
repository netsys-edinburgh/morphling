...
def assign_gemms_to_steps(
    ...
    result: dict[int, list[GemmCall]] = {i: [] for i in range(len(slots))}
    for g in gemms:
...
def write_report(
    ...
    total_wall_ns = 0
    if slots:
        total_wall_ns = slots[-1].end_ns - slots[0].start_ns

    total_violating_gemms = sum(s.num_violations for s in summaries)
    slots_with_v = len(summaries)

    pct_slots = (
        100.0 * slots_with_v / total_boundaries if total_boundaries else 0.0
    )
    pct_time = (
        100.0 * violation_time_ns / total_wall_ns if total_wall_ns else 0.0
    )

    out.write("=== SM Partition Violation Analysis ===\n\n")
    ...
    out.write(f"  Total wall time: {format_ns(total_wall_ns)}\n\n")
    out.write("Summary:\n")
    out.write(
    ...
    out.write(f"Per-step violations (top {len(show)} by overshoot):\n")
    for s in show:
        out.write(
    ...
    detail_count = 0
    out.write(f"Detailed violations (first {limit or 'all'}):\n")
    for s in ranked:
        for v in s.violations:
            out.write(
                f"  step={v.step} "
...
def write_json_summary(
    ...
    total_violations = int(sum(s.num_violations for s in summaries))
    avg_violation_duration_us = (
        float(violation_time_ns) / max(total_violations, 1) / 1000.0
    )
    ...
    result = {
        ...
        "total_violation_time_ms": float(violation_time_ns) / 1e6,
        "violations_per_step": violations_per_step,
        "avg_violation_duration_us": avg_violation_duration_us,
        "per_slot_summary": per_slot_summary,
        "total_slots": len(slots),
        "slots_with_violations": len(summaries),
        "violation_pct": 100.0 * len(summaries) / total_boundaries,
        "total_gemms": len(gemms),
        "violating_gemms": total_violations,
...
def main() -> None:
    ...
    if args.mode == "autograd":
        if not args.autograd_log:
            logger.error("--autograd-log is required in autograd mode")
            sys.exit(1)
        if not args.trace:
        ...
        if not autograd_path.exists():
            logger.error("Autograd log not found: %s", autograd_path)
            sys.exit(1)
        ...
        logger.info("parsing trace boundaries: %s", trace_path)
        ...
        if not step_path.exists():
            logger.error("Step boundaries not found: %s", step_path)
            sys.exit(1)
        ...
        logger.info("parsing step boundaries: %s", step_path)
        ...
        if args.trace:
            ...
            if trace_path.exists():
                ...
                logger.info("trace loaded: %d entries", len(trace))

    if len(slots) < 2:
        logger.warning("Need >= 2 steps for violation analysis")
    ...
    logger.info(
        "detecting violations (max_sm=%d)...",
        args.max_sm_count,
    )
    summaries = detect_violations(slots, step_gemms, args.max_sm_count)

    violation_time = compute_violation_time_ns(summaries, slots)
