...
def read_log(log_file: str) -> Tuple[List[VTEvent], List[ThroughputEvent], List[str]]:
...
def find_compute_start_times(vtime_events: List[VTEvent]) -> Dict[int, Dict[int, int]]:
    """
...
def calculate_offsets(
    compute_starts: Dict[int, Dict[int, int]],
    method: str = 'earliest'
    ...
    for gemm_id, device_times in compute_starts.items():
        ...
        if len(device_times) > 1:
            ...
            for device_id in sorted(device_times.keys()):
                ...
                status = "ahead" if offset < 0 else "behind" if offset > 0 else "aligned"
                print(f"  Device {device_id}: vt_start={vt:>10} us, offset={offset:>6} us ({status})")
...
def apply_offsets(
    ...
    for event in throughput_events:
        ...
        new_epoch_start = event.epoch_start_us + offset if event.epoch_start_us > 0 else event.epoch_start_us
        new_epoch_end = event.epoch_end_us + offset if event.epoch_end_us > 0 else event.epoch_end_us
    ...
    for event in adjusted_vtime_events:
        ...
        new_line = f"VTIME,{event.timestamp_us},{event.device_id},{event.gemm_id}," \
                  f"{event.phase},{event.event},{new_vt_start},{new_vt_end},{event.vt_duration_us}"
        synced_vtime_lines.append(new_line)
    
    for event in adjusted_throughput_events:
        new_epoch_start = event.epoch_start_us - min_vt if event.epoch_start_us > 0 else event.epoch_start_us
        new_epoch_end = event.epoch_end_us - min_vt if event.epoch_end_us > 0 else event.epoch_end_us
        
        new_line = f"{event.timestamp_us},{event.device_id},{event.gemm_id}," \
...
def main():
    ...
    write_synchronized_log(
        output_file, other_lines, synced_vtime_lines, synced_throughput_lines,
        vtime_events, throughput_events
    )
