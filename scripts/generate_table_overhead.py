...
def main():
    ...
    parser.add_argument(
        ...
        "--output",
        dest="output_tex",
        default="figures/table_overhead.tex",
    )
    parser.add_argument("--output-json", default="results/table_overhead.json")
    ...
    reduction = (
        f"{(1 - we_swaps / no_swaps) * 100:.0f}\\%" if no_swaps > 0 else "---"
    )
    rows.append(
        ("SM swaps per step", f"{no_swaps:.1f}", f"{we_swaps:.1f}", reduction)
    )
    ...
    reduction = (
        f"{(1 - we_viol / no_viol) * 100:.0f}\\%" if no_viol > 0 else "---"
    )
    rows.append(
        (
            "Isolation violations/step",
    ...
    no_avg_viol_us = get(no, "avg_violation_duration_us_mean", 0.0)
    we_avg_viol_us = get(we, "avg_violation_duration_us_mean", 0.0)
    ...
    table_data = {
        "rows": [
            {"metric": name, "no_ctrl": nc, "weaver": wv, "reduction": red}
            for name, nc, wv, red in rows
        ],
        "consistency_check": {
            "no_ctrl_combined": no_combined,
            "no_ctrl_swap_plus_viol": no_swap_ms + no_viol_ms,
            "within_5pct": abs(no_combined - (no_swap_ms + no_viol_ms))
