...
def analyze_gemm(before_events, after_events, gemm_id):
    ...
    before = {evt['device_id']: evt for evt in before_events.get(gemm_id, []) if evt['phase'] == 'COMPUTE' and evt['event'] == 'START'}
    after = {evt['device_id']: evt for evt in after_events.get(gemm_id, []) if evt['phase'] == 'COMPUTE' and evt['event'] == 'START'}
    ...
    print(f"{'Device':<10} {'原始 vt_start_us':<20} {'同步后 vt_start_us':<20} {'偏移量 (us)':<20}")
    print(f"{'-'*70}")
    ...
    print(f"\n【所有事件时间序列对比】\n")
    print(f"{'类型':<10} {'设备':<8} {'事件':<15} {'原始 vt':<20} {'同步后 vt':<20} {'变化':<15}")
    print(f"{'-'*100}")
    
    before_all = {(evt['device_id'], evt['phase'], evt['event']): evt for evt in before_events.get(gemm_id, [])}
    after_all = {(evt['device_id'], evt['phase'], evt['event']): evt for evt in after_events.get(gemm_id, [])}
    ...
    for key in sorted(before_all.keys()):
        ...
        if a_evt:
            ...
            print(f"{'✓':<10} {device_id:<8} {phase}/{event:<12} {before_vt:<20} {after_vt:<20} {change:+<15}")
...
def main():
    ...
    for gemm_id in before.keys():
        compute_starts_before = [evt for evt in before[gemm_id] if evt['phase'] == 'COMPUTE' and evt['event'] == 'START']
        compute_starts_after = [evt for evt in after[gemm_id] if evt['phase'] == 'COMPUTE' and evt['event'] == 'START']
        
        if compute_starts_before and compute_starts_after:
            before_dict = {evt['device_id']: evt['vt_start_us'] for evt in compute_starts_before}
            after_dict = {evt['device_id']: evt['vt_start_us'] for evt in compute_starts_after}
