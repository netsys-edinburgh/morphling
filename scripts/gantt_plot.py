...
def plot_gantt_chart(device_id, events, output_dir, time_range=None):
    ...
    for evt in events:
        ...
        if duration > 50000:  # Only if wider than 50k μs
            ax.text(bar_center, y, f'G{evt["gemm_id"]}', 
        ...
        if duration > 50000:
            ...
            ax.text(bar_center, y + bar_height/2 + 0.15, duration_label,
                   ha='center', va='bottom', fontsize=7, fontweight='bold',
                   color='#333333')
        
        # Add start time on the left side
        start_label = f'{evt["start"]}'
        ax.text(start, y - bar_height/2 - 0.15, start_label,
               ha='right', va='top', fontsize=7, fontweight='bold',
               color='#555555')
        
        # Add end time on the right side
        end_label = f'{evt["end"]}'
        ax.text(end, y - bar_height/2 - 0.15, end_label,
    ...
    ax.set_title(f'Device {device_id} Event Timeline (Gantt Chart)', 
    ...
    # Add legend
    legend_patches = [mpatches.Patch(color=EVENT_COLORS.get(phase, '#808080'), 
                                     label=phase) 
                     for phase in phases]
    ...
    stats_text = (f'Events: {len(events)}\n'
                  f'Time Span: {time_span/1e3:.0f}k μs')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 gantt_plot.py <log_file> [output_dir] [--gemm-range START:END]")
        print("Example: python3 gantt_plot.py perf_merged.log . --gemm-range 0:3")
        sys.exit(1)
    ...
    # Extract events
    print(f"Extracting events{'for gemm range ' + str(gemm_start) + ':' + str(gemm_end) if gemm_start else ''}...")
    ...
    time_range = (max(0, min_time - time_span * 0.05), max_time + time_span * 0.05)
