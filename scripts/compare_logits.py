...
def compare_logits(logits1: torch.Tensor, logits2: torch.Tensor) -> dict:
    ...
    with torch.no_grad():
        safe_logits2 = torch.where(torch.abs(logits2) < 1e-8, torch.ones_like(logits2) * 1e-8, logits2)
        rel_diff = torch.abs(diff / safe_logits2)
        rel_diff = torch.where(torch.isnan(rel_diff) | torch.isinf(rel_diff), torch.zeros_like(rel_diff), rel_diff)
    ...
    results["allclose"] = {
        "rtol_1e3_atol_1e3": torch.allclose(logits1, logits2, rtol=1e-3, atol=1e-3),
        "rtol_1e4_atol_1e4": torch.allclose(logits1, logits2, rtol=1e-4, atol=1e-4),
        "rtol_1e5_atol_1e5": torch.allclose(logits1, logits2, rtol=1e-5, atol=1e-5),
    ...
    if len(logits1.shape) >= 2:
        ...
        mean_abs_diff_per_token = torch.mean(torch.abs(logits1 - logits2), dim=-1)
...
def print_results(results: dict, logits1_path: str, logits2_path: str):
    ...
    print(f"  Mean:     {rel_stats['rel_mean']:.10f} ({rel_stats['rel_mean']*100:.6f}%)")
    print(f"  Std:      {rel_stats['rel_std']:.10f}")
    print(f"  Min:      {rel_stats['rel_min']:.10f}")
    print(f"  Max:      {rel_stats['rel_max']:.10f}")
    print(f"  Median:   {rel_stats['rel_median']:.10f} ({rel_stats['rel_median']*100:.6f}%)")
    ...
    for tolerance, data in close_tol.items():
        print(f"  Tolerance {tolerance}: {data['count']:>10} / {stats1['shape'][0] * stats1['shape'][1] * stats1['shape'][2] if len(stats1['shape']) == 3 else stats1['shape'][0] * stats1['shape'][1]:>10} ({data['percentage']:>6.2f}%)")
    ...
    for k in [1, 5, 10]:
        if f"topk_{k}" in results:
            ...
            print(f"  Top-{k:>2}: {topk_data['match_percentage']:>6.2f}% of positions have matching top-k tokens")
    ...
    if allclose["rtol_1e3_atol_1e3"]:
        ...
        print("⚠ Logits are VERY CLOSE (rtol=1e-4, atol=1e-4) but not identical")
    elif allclose["rtol_1e5_atol_1e5"]:
        print("⚠ Logits are CLOSE (rtol=1e-5, atol=1e-5) but have some differences")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Compare logits from different runs")
    parser.add_argument("--logits1", type=str, default="logits_comparison/logits_without_hooks.pt",
                        help="Path to first logits file (without hooks)")
    parser.add_argument("--logits2", type=str, default="logits_comparison/logits_with_hooks.pt",
                        help="Path to second logits file (with hooks)")
    parser.add_argument("--output", type=str, default="logits_comparison/comparison_report.json",
