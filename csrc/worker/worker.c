...
int dequeue_task(task_queue_t *task_queue) {
    ...
    if (task_index < 0 || task_index >= MAX_TASKS) {
        printf("[%s] Invalid task index %d retrieved from queue. Head: %d, Size: %d\n", get_timestamp(), task_index, head, current_size);
        return -1;
    }
...
void real_execute(sgemm_args_t *task, size_t offset, int task_index) {
    ...
    snprintf(log_buffer, sizeof(log_buffer), "GPU Memory: Free = %zu bytes, Total = %zu bytes", freeMem, totalMem);
    log_message(log_buffer);

    //log SGEMM parameters
    snprintf(log_buffer, sizeof(log_buffer), 
             "SGEMM Parameters - Task Index: %d, transa=%c, transb=%c, m=%d, n=%d, k=%d, alpha=%f, beta=%f, lda=%d, ldb=%d, ldc=%d",
             task_index, task->transa, task->transb, task->m, task->n, task->k, task->alpha, task->beta, task->lda, task->ldb, task->ldc);
    log_message(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Device pointer for A: %p", (void*)d_A);
    log_message(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Device pointer for B: %p", (void*)d_B);
    log_message(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Device pointer for C: %p", (void*)d_C);
    log_message(log_buffer);
    ...
    cudaMemGetInfo(&freeMem, &totalMem);
    snprintf(log_buffer, sizeof(log_buffer), "GPU Memory after copying matrices: Free = %zu bytes, Total = %zu bytes", freeMem, totalMem);
    log_message(log_buffer);
    ...
    cublasOperation_t transa = (task->transa == 'N' || task->transa == 'n') ? CUBLAS_OP_N : CUBLAS_OP_T;
    cublasOperation_t transb = (task->transb == 'N' || task->transb == 'n') ? CUBLAS_OP_N : CUBLAS_OP_T;

    //perform matrix multiplication
    log_message("Performing matrix multiplication with cuBLAS.");

    CHECK_CUBLAS_ERROR(cublasSgemm_v2(handle, 
                                   transa, transb, 
                                   task->m, task->n, task->k, 
                                   &task->alpha, d_A, task->lda, 
                                   d_B, task->ldb, 
                                   &task->beta, d_C, task->ldc));
    ...
    print_first_10_values("Matrix C after computation", task->c, task->m * task->n);
...
void *gpu_process(void *arg) {
    ...
    while (1) {
        ...
        if (task_index != -1) {
            ...
            char log_buffer[128];
            snprintf(log_buffer, sizeof(log_buffer), "Processing task at index %d.", task_index);
            log_message(log_buffer);

            if (task_index < 0 || task_index >= MAX_TASKS) {
                snprintf(log_buffer, sizeof(log_buffer), "Invalid task index %d detected. Skipping task.", task_index);
                log_message(log_buffer);
                continue;
            }
            ...
            snprintf(log_buffer, sizeof(log_buffer), "Task pointer address: %p", (void*)task_ptr);
            log_message(log_buffer);

            size_t task_offset = sizeof(sgemm_args_t);
            task_ptr->a = (float *)((char *)task_ptr + task_offset);
            task_ptr->b = (float *)((char *)task_ptr->a + task_ptr->lda * task_ptr->k * sizeof(float));
            task_ptr->c = (float *)((char *)task_ptr->b + task_ptr->ldb * task_ptr->n * sizeof(float));

            if ((char *)task_ptr->a < (char *)shared_mem_ptr || 
                (char *)task_ptr->a + task_ptr->lda * task_ptr->k * sizeof(float) > (char *)shared_mem_ptr + shm_size ||
                (char *)task_ptr->b < (char *)shared_mem_ptr || 
                (char *)task_ptr->b + task_ptr->ldb * task_ptr->n * sizeof(float) > (char *)shared_mem_ptr + shm_size ||
                (char *)task_ptr->c < (char *)shared_mem_ptr || 
                (char *)task_ptr->c + task_ptr->ldc * task_ptr->n * sizeof(float) > (char *)shared_mem_ptr + shm_size) {
                log_message("Error: Matrix pointer is outside of expected shared memory range. Exiting execution.");
                continue;
            }
