...
#define LOG_FILE "/home/eren/DeviceEmulator/csrc/intercept/logs/shared_memory.log"
...
shared_memory_t* initialize_shared_memory(size_t shm_size) {
    ...
    if (fd == -1) {
        ...
        snprintf(log_buffer, sizeof(log_buffer), "Shared memory object opened with file descriptor: %d", fd);
    ...
    if (ftruncate(fd, shm_size) == -1) {
        ...
        snprintf(log_buffer, sizeof(log_buffer), "Shared memory size set to: %zu bytes.", shm_size);
        log_message(log_buffer);
    }
    ...
    if (shared_mem_ptr == MAP_FAILED) {
        ...
        snprintf(log_buffer, sizeof(log_buffer), "Shared memory mapped at address: %p", (void*)shared_mem_ptr);
        log_message(log_buffer);
    }
...
shared_memory_t* attach_shared_memory(size_t shm_size) {
    ...
    char log_buffer[256];
    snprintf(log_buffer, sizeof(log_buffer), "Shared memory attached at address: %p", (void*)shared_mem_ptr);
    log_message(log_buffer);
