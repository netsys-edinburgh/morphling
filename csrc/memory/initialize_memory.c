...
shared_memory_t *initialize_shared_memory(size_t shm_size) {
    ...
    if (shared_mem_ptr == MAP_FAILED) {
        perror("mmap");
        snprintf(log_buffer, sizeof(log_buffer), "Failed to map shared memory. Size: %zu bytes.", shm_size);
    ...
    if (shared_mem_ptr->task_queue.initialization_flag != INITIALIZATION_FLAG) {
        ...
        snprintf(log_buffer, sizeof(log_buffer), "Shared memory and task queue initialized. Size: %zu bytes.", shm_size);
        log_message(log_buffer);
    } else {
        snprintf(log_buffer, sizeof(log_buffer), "Shared memory already initialized. Size: %zu bytes.", shm_size);
        log_message(log_buffer);
    }
