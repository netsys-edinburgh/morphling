...
void PinThreadToCore(int core_id) {
  ...
  if (ret == 0) {
    std::cout << "Thread " << tid << " pinned to CPU core " << core_id << std::endl;
  } else {
    std::cerr << "Failed to pin thread " << tid << " to core " << core_id << std::endl;
  }
}
...
int main(int argc, char* argv[]) {
  std::cout << "=== Multi-threaded Pin Thread Performance Test ===" << std::endl;
  ...
  std::cout << "Total time: " << std::fixed << std::setprecision(2) << total_time_s << " seconds" << std::endl;
  std::cout << "Overall throughput: " << std::setprecision(3) << throughput_gbs << " GB/s" << std::endl;
  std::cout << "Operations/sec: " << std::setprecision(0) << ops_per_sec << std::endl;
