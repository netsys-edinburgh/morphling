"""Network topology simulation for Morphling.

Provides NetworkTopology class for modeling network links between sites
with configurable bandwidth and latency. Includes helper functions for
building homogeneous and heterogeneous network meshes.
"""
...
class NetworkTopology:
    def __init__(self, config: NetworkConfig):
        self.config = config
        self.links = {
            (link.source_site, link.target_site): link for link in config.links
        }
    ...
    def transfer_time_ns(self, src: int, dst: int, data_bytes: int) -> int:
        ...
        return link.latency_ns + ceil(
            data_bytes * 8 / (link.bandwidth_gbps * 1e9) * 1e9
        )

    def reroute_network_cost_ns(
        self, src: int, dst: int, microbatch_activation_bytes: int
    ) -> int:
...
def build_default_mesh(
    num_sites: int, bandwidth_gbps: float = 10.0, latency_ns: int = 500_000
) -> NetworkConfig:
    links = []
...
def build_heterogeneous_mesh(site_configs: list[dict]) -> NetworkConfig:
    links = [
        NetworkLink(c["src"], c["dst"], c["bw"], c["lat"]) for c in site_configs
    ]
    return NetworkConfig(links)
