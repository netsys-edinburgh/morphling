"""Profile loading and generation for Morphling simulator.

Provides ProfileLoader for loading node profiling data from JSON and
helper functions for generating homogeneous profiles.
"""
...
class ProfileLoader:
    ...
    def get_node_profile(
        self, site_id: int, profiles: ProfilingData
    ) -> NodeProfile:
...
def build_homogeneous_profile(
    ...
    for i in range(num_sites):
        nodes.append(
            NodeProfile(
                i, "homogeneous", compute_sms, step_compute_ns, activation_bytes
            )
        )
