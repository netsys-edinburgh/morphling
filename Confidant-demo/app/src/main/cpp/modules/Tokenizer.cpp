...
bool SentencePiece::load(const std::string& filename) {
    ...
    while (std::getline(tok_file, line)) {
        ...
        SentencePieceStruct piece = {token, score, piece_type};
...
void SentencePiece::add_bos(std::vector<int> &ids) {
    int bosId = piece_to_id("<s>");
    if (!is_control(bosId)) {
        bosId = -1;
    }
...
void SentencePiece::add_eos(std::vector<int> &ids) {
    int eosId = piece_to_id("</s>");
    if (!is_control(eosId)) {
        eosId = -1;
    }
    ids.push_back(eosId);
...
int SentencePiece::get_vocab_size() const {
    return (int) sentence_pieces_.size();
}

int SentencePiece::get_eos_id() const {
    return piece_to_id("</s>");
}
